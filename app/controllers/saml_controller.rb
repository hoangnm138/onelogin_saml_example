...
class SamlController < ApplicationController
  ...
  def sso
    settings = SamlService.new.get_saml_settings(url_base)
    render action: :no_settings if settings.nil?
  ...
  def acs
    ...
    if response.is_valid?
      ...
      logger.info "Attributes: #{response.attributes.inspect}"
      render action: :index
    else
  ...
  def sp_logout_request
    ...
    settings = SamlService.new.get_saml_settings(get_url_base)
